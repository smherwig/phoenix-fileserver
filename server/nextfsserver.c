...
static void nextfs_server_socket_create(struct nextfs_server *server,
...
static void
nextfs_server_socket_create(struct nextfs_server *server, const char *udspath,
        bool anonymous)
{
    size_t pathlen = 0;
...
#define NEXTFSSERVER_USAGE \
    "usage: nextfsserver [options] UDSPATH IMAGE\n" \
    "\n" \
    "OPTIONS:\n" \
    "   -a\n" \
...
main(int argc, char *argv[])
{
    ...
    nextfs_bdconf_set_imagepath(bdconf, argv[1]);

    nextfs_server_open_block_device(server, bdconf);
    nextfs_server_socket_create(server, argv[0], anonymous);
